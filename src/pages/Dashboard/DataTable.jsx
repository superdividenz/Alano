...
const DataTable = ({ data, type, onDelete }) => {
  ...
  // Format special fields like status or timestamps
  const renderCell = (row, key) => {
    // Highlight paymentStatus or status fields
    ...
    if (row[key]?.toDate) {
      return row[key].toDate().toLocaleString();
    }
  ...
  const handleRespond = (row) => {
    if (row.email) {
      window.location.href = `mailto:${row.email}?subject=Response to your ${type}&body=Hi ${
        row.name || ""
      },`;
    } else if (row.phone) {
